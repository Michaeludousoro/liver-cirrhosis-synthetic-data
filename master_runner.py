...
Tabular GAN (cGAN), and a Tabular Variational Autoencoder (VAE).
...
How to run
----------
    Full run with paper-quality training (500 epochs for GAN, 300 for cGAN and VAE)
...
from src.synthetic_generator import VanillaGAN, cGAN, VAE, generate_synthetic
...
def main(args):
    ...
    print(f"\n  Training VAE for {args.epochs_tvae} epochs ...")
    tvae = VAE(
        latent_dim=32,
    ...
    print(f"  VAE generated {len(synthetic_tvae)} synthetic records")
    ...
    filtered_tvae  = iqr_result["filtered"]["VAE"]
    ...
    # 5.0 in standardised space. Before voting, the VAE pool is downsampled to
    # the size of the smallest adversarial pool so the higher-fidelity GAN and
    # cGAN are not swamped by the far larger VAE pool (equalisation).
    ...
    print(f"  Equalisation: VAE pool downsampled from {len(filtered_tvae)} "
    ...
    if len(consensus_df) == 0:
        ...
        raise RuntimeError("Consensus failed; Scenario C cannot be replaced with cGAN data.")
    ...
    plot_iqr_filtering(
        ...
        {"GAN": filtered_gan, "cGAN": filtered_ctgan, "VAE": filtered_tvae}
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Training epochs for VAE (default 300)"
    )
    parser.add_argument(
