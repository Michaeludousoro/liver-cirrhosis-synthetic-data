...
``Evaluating Synthetic Liver Cirrhosis Data: Predictive Utility, Distributional Fidelity, and Consensus-Based Quality Filtering''}
...
Training curves for all three models are shown in Fig.~\ref{figS:training}. These curves describe optimisation behaviour but do not establish convergence or exclude mode collapse.



\subsection{Reproducibility}

The project uses fixed seeds for Python, NumPy, TensorFlow, and the train/test split. The original pipeline and subsequent evaluation scripts generate the saved result tables. The recorded environment specifies Python 3.9.6 and TensorFlow 2.15. Reproducibility should be checked against the exact script sequence and software versions; fixed seeds do not guarantee bitwise-identical results across hardware or thread configurations.
...
\begin{table}[!t]
  \centering
  \caption{Approximate DP-SGD Accounting Projections (Not Certified) at $\delta = 10^{-5}$ for $n = 193$ Training Patients}
  \label{tab:dpsgd}
  \begin{tabular}{ccl}
    \toprule
...
The projections depend on the assumed sampling and composition settings. They do not establish a minimum cohort size for differential privacy, and there is no universal numerical threshold separating meaningful from meaningless protection. A valid guarantee requires a correctly implemented private training mechanism and validated accounting.

Output perturbation is an empirical proximity experiment and must be distinguished from differentially private training. Neither the perturbation results nor these projections certify the release of the present synthetic records.
