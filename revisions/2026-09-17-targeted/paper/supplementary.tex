...
\begin{document}
\title[Supplementary Material]{Supplementary Material: Theory of Augmentation for Improved Prediction Accuracy: Evaluating the Predictive Utility of Synthetic Liver Cirrhosis Data with Consensus-Based Quality Filtering}
\author{M. Mursaleen}
...
\email{michaeludousoro13@gmail.com}
...
\affiliation{\institution{University of Roehampton}\department{School of Computing, Engineering and the Built Environment}\city{London}\country{United Kingdom}}
\renewcommand{\shortauthors}{Mursaleen et al.}
...
\rev{The patient identifier column is discarded as it carries no clinical signal. The remaining 19 columns comprise 11 continuous laboratory measurements, five binary clinical indicators, two ordinal variables, and the binary survival outcome. Categorical encodings are given in Section~3 of the main paper.}

\rev{For the full-training generative analysis, min-max scaling is fitted to the 193 real training patients and inverted after generation. The synthetic table has 19 columns: 18 generated variables and Status. Classifiers exclude N\_Days and use 17 baseline predictors, with separate training-fitted standardisation. In the corrected cross-validation, every scaler, generator, IQR bound, and consensus pool is fitted or constructed anew inside the corresponding training fold.}
...
\Description{Supporting figure: figS01 correlation matrix. See the caption for the analysis and its limitations.}
...
\begin{table}[!t]
  ...
  \begin{tabular}{llll}
    ...
    \rev{Output dimension}      & \rev{19}           & \rev{19}                & \rev{19} \\
...
\rev{The model labelled cGAN is a conditional GAN in the sense of Mirza and Osindero \cite{mirza2014}: it conditions generator and discriminator on the outcome label and nothing more. It is deliberately not called CTGAN, because it implements none of the three components that define the Conditional Tabular GAN of Xu et al.\ \cite{xu2019}, namely mode-specific normalisation, training-by-sampling, and a PacGAN critic with the Wasserstein gradient penalty. A full CTGAN was implemented separately and compared against it; that comparison is reported in Section~4 of the main paper, where CTGAN reaches FID 0.108 against 0.072 for the cGAN after 4{,}000 generator updates, with convergence not established.}
...
\rev{Training curves for all three models are shown in Fig.~\ref{figS:training}. These curves describe optimisation behaviour but do not establish convergence or exclude mode collapse.}
...
\Description{Supporting figure: figS02 training losses. See the caption for the analysis and its limitations.}
\caption{\revisioncolor Saved full-training loss curves for the three core generative models. They describe optimisation, not demonstrated convergence, and are not loss curves from the corrected cross-validation rerun.}
\label{figS:training}
\end{figure}

\subsection{Reproducibility}

\rev{The full-training artifact analyses use seed 42. The corrected five-fold generator rerun uses seeds 42--46, with split and classifier seeds fixed at 42. Its environment includes Python 3.9.6, TensorFlow 2.15.0, NumPy 1.26.4, and scikit-learn 1.5.2. The run completed 15 generator fits at the original epoch settings. Fold membership, scaling extrema, IQR bounds, synthetic records, and individual fold AUCs are retained. Fixed seeds alone do not ensure identical results across software versions or hardware.}

\rev{Two practical notes for anyone reproducing this work. First, the consensus procedure used throughout is the \emph{equalised} variant described in Section~3 of the main paper, in which the VAE pool is downsampled to match the smallest adversarial pool before voting. An earlier non-equalised variant produces a differently sized consensus set and different downstream numbers. Second, the nearest-neighbour privacy analysis reads the datasets written by \texttt{master\_runner.py} rather than regenerating them, so it always describes the same synthetic records the rest of the results are computed from.}
...
\rev{The VAE has small absolute standardised mean differences on all 11 variables (maximum 0.230), but it is not the closest method for every variable. For example, its Albumin and SGOT differences exceed those of cGAN. Small mean differences should not be interpreted as agreement of the entire distribution.}

\rev{One possible interpretation is that concentration around central values preserves means while reducing variability. This is not a demonstrated mechanism or an inevitable property of VAEs. The KS and Jensen--Shannon results reveal distributional differences not captured by standardised mean differences.}

\rev{The results illustrate that quality rankings depend on the selected criterion. Mean agreement, distributional-shape agreement, and predictive usefulness are different properties and should be measured separately. This dataset does not establish a universal ranking of generative models.}

\subsection{Supporting Figures}

\rev{Fig.~\ref{figS:iqr} shows the IQR-based Tukey fence filter (Section~3 of the main paper) in action on the Bilirubin distribution, the hardest continuous feature to reproduce faithfully.}
...
\Description{Supporting figure: figS03 distribution comparison. See the caption for the analysis and its limitations.}
...
\Description{Supporting figure: figS04 correlation heatmap. See the caption for the analysis and its limitations.}
...
\rev{The lower FID after perturbation is consistent with a change in the moments of the synthetic distribution, but it does not establish the mechanism responsible or demonstrate improved downstream utility. Noise can also produce implausible clinical combinations, so plausibility and predictive performance need separate reassessment.}

\rev{The $\sigma=0.7$ setting is reported as an illustrative point in the sweep. A near-duplicate rate of 1.4\% is not a validated release criterion or formal disclosure guarantee.}

\subsection{Illustrative Privacy-Accounting Projections}

\rev{Table~\ref{tab:dpsgd} reports approximate analytical projections from a simplified DP-SGD accounting formula at $\delta=10^{-5}$ for 193 training records. No differentially private generator was trained for this sweep. The approximation is not a validated accountant and the numbers must not be interpreted as certified privacy budgets.}

\begin{table}[!t]
  \centering
  \caption{\revisioncolor Approximate DP-SGD Accounting Projections (Not Certified) at $\delta = 10^{-5}$ for $n = 193$ Training Patients}
  \label{tab:dpsgd}
  \begin{tabular}{ccl}
    \toprule
    Noise multiplier & $\varepsilon$ & Interpretation \\
    \midrule
    0.5 & 210.4 & Unvalidated projection \\
    0.8 & 89.2 & Unvalidated projection \\
    1.0 & 61.2 & Unvalidated projection \\
    1.2 & 46.0 & Unvalidated projection \\
    1.5 & 33.6 & Unvalidated projection \\
    2.0 & 23.9 & Unvalidated projection \\
    3.0 & 14.0 & Unvalidated projection \\
    \bottomrule
  \end{tabular}
\end{table}

\rev{The projections depend on the assumed sampling and composition settings. They do not establish a minimum cohort size for differential privacy, and there is no universal numerical threshold separating meaningful from meaningless protection. A valid guarantee requires a correctly implemented private training mechanism and validated accounting.}

\rev{Output perturbation is an empirical proximity experiment and must be distinguished from differentially private training. Neither the perturbation results nor these projections certify the release of the present synthetic records.}
...
\rev{These plots display the saved held-out classification results, which were reproduced at their reported precision in the artifact audit. They are not cross-validation plots; the corrected fold-contained results are reported separately in the main manuscript.}

\rev{Figure~\ref{figS:prcurves} compares held-out precision--recall curves. Their interpretation depends on the event prevalence in the evaluation cohort, not only training-set balance. Visual proximity of curves does not demonstrate equivalence or establish a null effect.}

\begin{figure}[!tb]
  \centering
  \includegraphics[width=\linewidth]{figS06_pr_curves.png}
\Description{Supporting figure: figS06 pr curves. See the caption for the analysis and its limitations.}
...
\rev{Figure~\ref{figS:auc} compares held-out AUC across Scenarios A--C. All displayed AUCs exceed 0.80, but this threshold does not establish clinical adequacy or prove that augmentation cannot harm a model. The changes differ by classifier and should be interpreted with their uncertainty.}

\begin{figure}[!tb]
  \centering
  \includegraphics[width=\linewidth]{figS07_auc_comparison.png}
\Description{Supporting figure: figS07 auc comparison. See the caption for the analysis and its limitations.}
...
\Description{Supporting figure: figS08 performance heatmap. See the caption for the analysis and its limitations.}
...
\Description{Supporting figure: figS09 model comparison. See the caption for the analysis and its limitations.}
...
\Description{Supporting figure: figS10 all metrics. See the caption for the analysis and its limitations.}
...
\rev{Section~4 of the main paper reports log-rank test statistics comparing each generator's synthetic (N\_Days, Status) distribution against the real training cohort, as a second check on whether FID predicts downstream utility. Fig.~\ref{figS:kmcomparison} gives the underlying Kaplan-Meier curves for all six generators.}

\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.98\textwidth]{figS12_km_comparison.png}
\Description{Supporting figure: figS12 km comparison. See the caption for the analysis and its limitations.}
  \caption{\revisioncolor Kaplan-Meier survival curves for each generator's synthetic (N\_Days, Status) pool (red) against the real training cohort (black), with log-rank test $p$-values. The comparisons are exploratory, unadjusted for multiplicity, and dependent on the reference cohort used for generation. Non-significant tests do not establish equivalent survival distributions.}
...
\Description{Supporting figure: figS13 iqr filtering. See the caption for the analysis and its limitations.}
  \caption{\revisioncolor Effect of IQR-based Tukey fence filtering on the Bilirubin distribution for all three generative models. Grey bars show real patient density. Pink bars show unfiltered synthetic distribution; coloured bars show the post-filter distribution. The GAN and cGAN produce records extending beyond 15~mg/dL that the filter removes. The VAE retains 499 of 500 generated records but produces a distribution shifted rightward relative to real patients.}
  \label{figS:iqr}
\end{figure*}
...
\Description{Supporting figure: figS14 privacy risk analysis. See the caption for the analysis and its limitations.}
  \caption{\revisioncolor Privacy disclosure risk analysis using nearest-neighbour distances in standardised feature space. Left: violin plots of distances from each synthetic record to its closest real training patient. The red dashed line marks the risk threshold at the 5th percentile of real-to-real distances. GAN and cGAN distributions are centred above this threshold. Right: near-duplicate rates by method. The observed rates are 7.3\% for GAN, 9.6\% for cGAN, 35.7\% for VAE, and 21.1\% for consensus. Lower proximity rates do not certify safe release.}
...
\Description{Supporting figure: figS15 privacy enhancement. See the caption for the analysis and its limitations.}
  \caption{\revisioncolor VAE privacy enhancement analysis. Left: near-duplicate rate versus Gaussian output noise $\sigma$ (in feature standard-deviation units); at $\sigma = 0.7$ the rate falls from 35.7\% to 1.4\%, crossing the 5\% near-duplicate target. Centre: tabular FID score versus $\sigma$; FID improves from 0.228 to 0.0481, below the equalised consensus benchmark of 0.0809. Right: estimated DP-SGD privacy budget $\varepsilon$ versus noise multiplier at $\delta = 10^{-5}$; $\varepsilon$ remains above 14 even at a multiplier of 3.0, under the illustrative approximation; this is not a trained or certified DP model.}
  \label{figS:privacyenhance}
\end{figure*}

\begin{figure}[!tb]
  \centering
  \includegraphics[width=\linewidth]{figures/fig14_study_overview.png}
\Description{Supporting figure: figures/fig14 study overview. See the caption for the analysis and its limitations.}
  \caption{\revisioncolor Updated evaluation pipeline. The 276 complete cases are split into 193 training and 83 held-out patients before fitting preprocessing or generators. The corrected cross-validation independently refits generators, filters, and consensus inside each training fold; the test cohort is used only for evaluation.}
  \label{figS:pipeline}
\end{figure}
